...
# Generate predicted ordering for our hand-coded binoms

```{r}
hand_coded_corpus = read_csv("../Data/attested_binoms_train.csv") %>%
  mutate(LogCenteredOverallFreq = as.numeric(scale(OverallFreq))) %>%
  mutate(
    GenPref = GenPref - 0.5,
    RelFreq = RelFreq - 0.5
...
#   Estimate   Est.Error     Q2.5    Q97.5
# on the response scale (probabilities, because family = bernoulli)
hand_coded_corpus_finetuning = cbind(hand_coded_corpus, pred_summ) %>%
  mutate(item = row_number())

hand_coded_corpus_finetuning = hand_coded_corpus_finetuning %>%
  uncount(weights = 10, .remove = F) %>%
  group_by(item) %>%
  mutate(predicted_resp_value = rbinom(n(), size = 1, prob = Estimate)) %>%
  ungroup() 
...
hand_coded_corpus_finetuning = hand_coded_corpus_finetuning %>%
  rename(WordA = Word1, WordB = Word2)
...
#write_csv(hand_coded_corpus_finetuning, '../Data/hand_coded_corpus_finetuning_with_model_preds.csv')
```
