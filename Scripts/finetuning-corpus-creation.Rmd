...
selected_binoms = all_binoms %>%
  slice_sample(n = 1000000, replace = F) 
...
compute_bstress <- function(cmuA, cmuB) {
  ...
  # If either word has no vowel stress info, return NA
  if (is.na(sA) || is.na(sB)) return(NA_integer_)

  if (sA > 0 && sB <= 0) return(1L)
  if (sA <= 0 && sB > 0) return(-1L)
  0L
}
...
nrow(binoms_filtered)

binoms_filtered = binoms_filtered %>%
  filter(WordA != WordB)
#write_csv(binoms_filtered, "../Data/fintetuning-all-binoms-without-constraints-coded.csv")
...
binoms_with_genpref = read_csv('../Data/genpref-data-sampled-without-sentences.csv') %>%
  mutate(Freq = log(WordA_Unigram) - log(WordB_Unigram),
         Len = Length,
         '*BStress' = BStress) %>%
  mutate(Alpha = paste0(WordA, " and ", WordB))
...
test = read_csv("../Data/finetuning-binoms-with-constraints-coded.csv")
...
#write_csv(finetuning_corpus_with_model_preds, '../Data/oss120b-coded-genpref-data-sampled-without-sentences.csv')
...
```{r}
all_binoms_data = read_csv('../Data/all-binoms-data-sampled-without-sentences.csv')

all_binoms_data = all_binoms_data %>%
  ungroup() %>%
  mutate(RelFreq = AlphaOrderFreq / TotalFreq,
         OverallFreq = TotalFreq,
         binom_prob = TotalFreq / sum(TotalFreq))
...
#   Estimate   Est.Error     Q2.5    Q97.5
# on the response scale (probabilities, because family = bernoulli)
all_binoms_with_model_preds = cbind(all_binoms_data, pred_summ) %>%
  na.omit() %>%
  mutate(item = row_number()) %>%
  mutate(predicted_resp_value = rbinom(n(), size = 1, prob = Estimate)) %>%
  mutate(predicted_resp = case_when(
...
write_csv(all_binoms_with_model_preds, '../Data/all_binoms_with_model_preds.csv')
...
```{r}
relfreq_data = binoms_with_genpref %>%
  mutate(item = row_number()) %>%
  uncount(num_sentences) %>%
  mutate(predicted_resp_value = rbinom(n(), size = 1, prob = (RelFreq+0.5))) %>%
  mutate(predicted_resp = case_when(
    predicted_resp_value == 1 ~ paste0(WordA, ' and ', WordB),
    predicted_resp_value == 0 ~ paste0(WordB, ' and ', WordA)
  ))
...
write_csv(relfreq_data, '../Data/oss120b-coded-relfreq-data-sampled-without-sentences.csv')
```
