...
run_pythia_pipeline = function(
  ...
  save_stub,
  backend = "brms",
  force_handcoded = FALSE   # 👈 ADD THIS
) {
  ...
  human_novel =
    ...
    mutate(
      !!slope_var := as.numeric(.data[[slope_var]])
    )
...
for (m in baseline_models) {
  ...
  all_results[[paste0(m, "__handcoded")]] <-
    tryCatch(
      run_pythia_pipeline(
        model_name = m,
        all_model_preds = all_model_preds,
        ...
        save_stub = stub_hc,
        backend = backend_choice,
        force_handcoded = TRUE     # 👈 THIS IS THE ONLY DIFFERENCE
      ),
      error = function(e) {
        message("❌ ERROR in baseline handcoded ", m, ": ", e$message)
        list(error = TRUE)
      }
    )
}
...
write_csv(coef_table_attested, '../Data/coef_table_attested_lmer.csv')
write_csv(coef_table_novel, '../Data/coef_table_novel_lmer.csv')
...
coef_aug_att <- coef_table_attested %>%
  mutate(
    ...
    # HANDCODED FLAG — ANY mention of "handcoded"
    handcoded_flag =
...
coef_aug_novel <- coef_table_novel %>%
  mutate(
    model_size = case_when(
      str_detect(model, regex("70m",  ignore_case=TRUE))  ~ "70m",
      str_detect(model, regex("160m", ignore_case=TRUE)) ~ "160m",
...
```{r}
coef_with_improvement_att = coef_with_improvement_att %>%
  mutate(tmp = str_split(model, "[/_]")) %>%
  unnest_wider(tmp, names_sep = "_")

coef_with_improvement_novel = coef_with_improvement_novel %>%
  mutate(tmp = str_split(model, "[/_]")) %>%
  unnest_wider(tmp, names_sep = "_")
...
model_list_baby_lms = all_model_preds %>%
  filter(str_detect(model, "babylm")) %>%
  distinct(model) %>%
  pull(model)
...
all_results_babylm = vector("list", length(model_list_baby_lms))
names(all_results_babylm) = model_list_baby_lms

for (m in model_list_baby_lms) {
...
for (m in baseline_models) {
  ...
  all_results_babylm[[paste0(m, "__handcoded")]] <-
    tryCatch(
      run_pythia_pipeline(
        model_name = m,
        all_model_preds = all_model_preds,
...
coef_table_attested_babylm = map_df(names(all_results_babylm), function(m) {
  res = all_results_babylm[[m]]
  out = extract_slope(res, which = "attested")
  if (is.null(out)) return(NULL)
  out$model = m
  out
...
coef_table_novel_babylm = map_df(names(all_results_babylm), function(m) {
  res = all_results_babylm[[m]]
  out = extract_slope(res, which = "novel")
  if (is.null(out)) return(NULL)
  out$model = m
...
loo_compare(
  human_data_novel_best_pythia_genpref_model,
  human_data_novel_best_base_pythia_model
)

loo_compare(
  human_data_attested_best_pythia_genpref_model,
  human_data_attested_hand_base_model
)

```
## BabyLM model comparisons

```{r}
#best babylm model: qing-yao/handcoded_nunique_nb50k_babylm_ep5_lr1e-4_seed42

res_handcoded <- run_pythia_pipeline(
  model_name = "qing-yao/handcoded_nunique_nb50k_babylm_ep5_lr1e-4_seed42",
  all_model_preds,
  corpus,
  all_human_data_novel,
  all_human_data_attested,
  test_df,
  save_stub = "../Data/babylm_finetuned_best",
  backend = "brms",
  force_handcoded = TRUE     # 👉 forces Alpha subset = test_df
)

res_genpref <- run_pythia_pipeline(
  model_name = "qing-yao/binomial-babylm-base_seed-42_1e-3",
  all_model_preds,
  corpus,
  all_human_data_novel,
  all_human_data_attested,
  test_df,
  save_stub = "../Data/babylm_base",
  backend = "brms",
  force_handcoded = TRUE     # 👉 IMPORTANT: same constraint here
)
...
human_data_novel_joint <-
  all_human_data_novel %>%
  semi_join(res_handcoded$item_novel, by="Alpha") %>%
  semi_join(res_genpref$item_novel, by="Alpha") %>%
  left_join(res_handcoded$item_novel, by="Alpha") %>%
  left_join(res_genpref$item_novel, by="Alpha") %>%
  mutate(
    z_pythia_handcoded =
      scale(item_logodds_pythia_handcoded)[,1],
    z_pythia_genpref =
      scale(item_logodds_pythia_genpref)[,1]
  )

human_data_attested_joint <-
  all_human_data_attested %>%
  semi_join(res_handcoded$item_attested, by="Alpha") %>%
  semi_join(res_genpref$item_attested, by="Alpha") %>%
  left_join(res_handcoded$item_attested, by="Alpha") %>%
  left_join(res_genpref$item_attested, by="Alpha") %>%
  mutate(
    z_pythia_handcoded =
      scale(item_logodds_pythia_handcoded)[,1],
    z_pythia_genpref =
...
human_data_novel_joint_model <- brm(
  resp_binary ~ z_pythia_handcoded + z_pythia_genpref +
    (1 | participant) + (1 | Alpha),
  ...
  save_pars = save_pars(all = TRUE),
  file = "../Data/human_data_novel_joint_model"
)
...
human_data_attested_joint_model <- brm(
  ...
  iter = 6000, warmup = 3000,
  save_pars = save_pars(all = TRUE),
  file = "../Data/human_data_attested_joint_model"
)
